...
$recordset = mysqli_query($conn, "SELECT * FROM Artists WHERE ArtistName = '" . $ArtistName . "'");
$result = $recordset->fetch_assoc();
$ArtistID = $result["ID"]
$Description = $result["Description"];
$YearLived = $result["YearLived"];
$recordset->free();
?>
...
<html>
	<head>
		<title> <?php echo $ArtistName; ?> | Explore Classical</title>
		...
		<div class="container-fluid">		
			<div class="row">
				...
				<div class="col-sm-8">
					...
					<?php 
						$counter = 0
						$recordset = mysqli_query($conn, "SELECT * FROM Albums WHERE ArtistID = '" . $ArtistID. "'");
						if($recordset->num_rows > 0){
							while($result = $recordset->fetch_assoc()){
								if($counter % 2 == 0){
									echo '<a href="Album.php?ID='.$result[TrackID].'" class="list-group-item active">';
									$counter = $counter + 1;
								} else {
									echo '<a href="Album.php?ID='.$result[TrackID].'" class="list-group-item">';	
									$counter = $counter + 1;
								}
