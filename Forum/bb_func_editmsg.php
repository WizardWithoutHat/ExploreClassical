...
if ($user_id!=0 and $row=db_simpleSelect(0, "$Tp,$Tt", "$Tp.post_text, $Tt.topic_title, $Tp.post_time, $Tt.topic_status, $Tt.forum_id", "$Tp.post_id", '=', $post, '', '', "$Tp.topic_id", '=', "$Tt.topic_id") and $row[4]==$forum and (!isset($useredit) or $useredit==0 or $useredit>(strtotime('now')-strtotime($row[2])) or $user_id==1 or $isMod==1) and ($userAllow==$user_id or $user_id==1 or $isMod==1) ) {
...
if($whoEdited==2 or $whoEdited==3) $ch=' checked="checked"'; else $ch='';
