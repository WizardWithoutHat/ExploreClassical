...
Copyright (C) 2013, 2014 Paul Puzyrev. www.minibb.com
Latest File Update: 2014-Mar-16
*/
...
if(!isset($GLOBALS['archives'])) $result=mysql_query('select '.$what.' from '.$GLOBALS['Tu'].' LEFT JOIN '.$GLOBALS['Tp'].' ON '.$GLOBALS['Tu'].'.'.$GLOBALS['dbUserId'].'='.$GLOBALS['Tp'].'.poster_id where '.$GLOBALS['Tp'].'.poster_id IS NULL order by '.$GLOBALS['Tu'].'.'.$GLOBALS['dbUserId'].' '.$GLOBALS['makeLim']);
else $result=mysql_query("select {$what} from {$GLOBALS['Tu']} where num_posts=0 and num_topics=0 order by {$GLOBALS['dbUserId']} ASC {$GLOBALS['makeLim']}");
if($result) {
$GLOBALS['countRes']=mysql_num_rows($result);
$GLOBALS['result']=$result;
}
